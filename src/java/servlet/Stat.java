    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (request.getParameter("debut") == null) {
                // Exécute la requête et la sauvegarde
                String[] stats = dao.stats(Integer.parseInt(session.getAttribute("id").toString()));
                String gsonData = gson.toJson(stats);
                // Données de la base
                out.println(gsonData);
            } else {
                // Exécute la requête et la sauvegarde
                String[] stats = dao.statsPeriode(Integer.parseInt(session.getAttribute("id").toString()), request.getParameter("debut"), request.getParameter("fin"));
                String gsonData = gson.toJson(stats);
                // Données de la base
                out.println(gsonData);
            }
